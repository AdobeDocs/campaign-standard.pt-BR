...
## Configurando a integração de dados de Pontos de Campanha de interesse com SDKs Adobe Experience Platform {#configuring-campaign-poi-aep-sdk}
...
1. Em seguida, em **[!UICONTROL Adobe Experience Platform Launch]**, é necessário criar regras para suportar casos de uso móvel entre Ponto de interesse e Adobe Campaign.\
   Essa regra será acionada quando um usuário inserir um **[!UICONTROL Point of Interest]** circunscrito geograficamente. Consulte [page](https://helpx.adobe.com/campaign/kb/config-app-in-launch.html#Locationpostback) para criar sua regra.

1. Defina **[!UICONTROL Points of Interest]** em Locais. Consulte [Criar um Ponto de Interesse](https://docs.adobe.com/content/help/en/places/using/poi-mgmt-ui/create-a-poi-ui.html).

1. Certifique-se de acessar o aplicativo móvel e os dados de localização coletados no Adobe Campaign. Consulte [Acessar aplicativos móveis usados para coletar dados de localização](#accessing-mobile-apps-used-to-collect-location-data) e [Acessar dados de localização coletados](#accessing-collected-location-data).

## Configurando a integração de dados de Pontos de Campanha de interesse usando o SDK V4 {#configuring-campaign-poi-sdkv4}
...
1. Configure seu aplicativo móvel no Adobe Campaign. Consulte [Configurar um aplicativo móvel na Campanha](#setting-up-a-mobile-app-in-campaign).
...
1. Execute a configuração específica do aplicativo móvel:
   ...
   * Integre o SDK do Experience Cloud Mobile ao seu aplicativo móvel. Consulte [Integração do SDK em um aplicativo móvel](#integrating-the-sdk-into-a-mobile-application).

1. Defina Pontos de interesse na interface do Adobe Mobile Services. Consulte [Definindo Pontos de interesse no Adobe Mobile Services](#defining-points-of-interest-in-adobe-mobile-services).
...
1. Digite um nome no campo **[!UICONTROL Application name]** e clique em **[!UICONTROL Create]**.
...
* O URL **[!UICONTROL Collect PII endpoint]** é usado para coletar as IDs de Experience Cloud dos usuários e os tokens de registro do aplicativo móvel quando ele é iniciado. Quando um usuário entra no aplicativo usando credenciais como email, nome, sobrenome etc., esses dados também são coletados e usados para reconciliar o token de registro do usuário com um perfil Adobe Campaign.
* O URL **[!UICONTROL Location Services endpoint]** é usado para coletar dados de localização, como latitude, longitude e raio de um usuário de um Ponto de interesse.
...
1. Na seção **Serviço de ID de Visitante**, marque **Ativar** e selecione a organização na lista suspensa. Clique em **Save**.
...
1. Crie um postback.
   ...
   * No campo **[!UICONTROL URL]**, copie o URL **[!UICONTROL Collect PII Endpoint]** do aplicativo móvel que você configurou na interface do Adobe Campaign, precedido pelo nome do servidor. Consulte [Configurar um aplicativo móvel na Campanha](#setting-up-a-mobile-app-in-campaign).
   * Preencha o campo **[!UICONTROL Post Body]** da seguinte forma:
...
1. Crie um segundo postback.
   ...
   * No campo **[!UICONTROL URL]**, copie o URL **[!UICONTROL Location Services Endpoint]** do aplicativo móvel que você configurou na interface do Adobe Campaign, precedido pelo nome do servidor. Consulte [Configurar um aplicativo móvel na Campanha](#setting-up-a-mobile-app-in-campaign).
   * Preencha o campo **[!UICONTROL Post Body]** da seguinte forma:
...
Esta etapa está descrita em [page](https://helpx.adobe.com/br/campaign/kb/configuring-app-sdkv4.html).
...
Esta etapa é descrita na página [Configurar um aplicativo móvel usando o SDK V4](https://helpx.adobe.com/campaign/kb/configuring-app-sdkv4.html).
...
Quando a configuração for concluída, os dados coletados de Pontos de interesse serão listados na guia **[!UICONTROL Places]** de cada perfil. Para acessar a lista:
