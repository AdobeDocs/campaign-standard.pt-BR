...
<p>O Adobe Campaign agora audita ações de criação, edição e exclusão para: fluxos de trabalho, opções e recursos personalizados. As modificações desses itens também são rastreadas.</p>
...
<tr> 
<td>
<p>Agora você pode executar fluxos de trabalho da campanha no modo de diagnóstico. Esse modo registra informações para ajudar a solucionar problemas de execução. Todo o plano de execução é registrado se uma consulta de fluxo de trabalho levar, por padrão, mais de um minuto.</p>
...
* O limite de conexão de banco de dados agora está alinhado ao pacote de campanha para evitar erros de conexão.
...
* Em entregas com muitas variantes, os usuários não poderão mais criar cópias de idioma se a variante padrão tiver sido excluída. Uma mensagem agora é exibida durante a criação da cópia no idioma de destino. (CAMP-48235)
...
* Correção de um problema que impedia o envio de um relatório por meio do menu **Compartilhar** > **Enviar relatório agora** ou **Enviar relatório pelo cronograma**. (CAMP-47798)
...
* Correção de um problema que resultava em falha nas chamadas de APIs ao ser criado um novo recurso personalizado e ao ser habilitada a opção **Não sincronizar**. (CAMP-48014)

* Correção de um problema em que os recursos personalizados com a opção **Não sincronizar** habilitada podiam fazer referência a um esquema que tinha sido reformulado ou excluído. Esse problema causava um erro durante a publicação dos recursos personalizados.
...
* Correção de um problema que impedia a exibição da miniatura de visualização no resumo da entrega ao ser criado um email multilíngue com a integração do Adobe Experience Manager. Esse problema ocorria ao ser usado o botão **Criação de cópia no idioma de destino** para criar as variantes de email. (CAMP-47810)

* Correção de um problema que impedia os usuários de acessar a entrega pai por meio da entrega filha de email ou SMS. (CAMP-47986)
...
* Correção de um problema em que os relatórios não eram recebidos após o uso da opção **Enviar relatório pelo cronograma**. (CAMP-48583, CAMP-47786)
...
* Correção de um problema com o teste AB que impedia que o conteúdo da variante B fosse atualizado após a modificação do modelo da entrega. (CAMP-45235)
...
* Correção de um problema em fluxos de trabalho que impedia a exibição de mensagens de falha nos logs da entrega após a adição da coluna **Motivo** à guia de dados adicional. (CAMP-45139)
...
* Correção de um problema que resultava em problemas de lentidão ao arrastar e soltar atividades em um fluxo de trabalho contendo uma grande quantidade de atividades **Consulta** e **Ler público-alvo**. (CAMP-44511)
...
* Correção de um problema que impedia a execução de uma atividade **Ler público-alvo** se ela estivesse configurada com um nome de público-alvo dinâmico. (CAMP-46047)
...
* Correção de um problema que impedia a exibição de uma visualização detalhada específica para um perfil. Isso ocorria se o recurso de perfil tivesse sido estendido com campos personalizados com a opção **Adicionar uma seção de campos personalizados** habilitada.
...
* Correção de um problema ao usar uma atividade **Segmentação** com entregas recorrentes em um fluxo de trabalho que fazia com que a entrega fosse enviada para o público-alvo errado. (CAMP-46275, CAMP-46470)
...
* Correção de um erro que ocorria quando a configuração de cronograma era atualizada manualmente com o teclado nos parâmetros de cronograma da entrega de email.
...
* Correção de um problema que deixava o campo de extensão **Serviço** vazio e impossível de ser definido nas **Propriedades de email**, mesmo que estivesse definido no modelo da entrega.
...
* Correção de um problema que impedia a importação de conteúdo HTML em um modelo de mensagem transacional. (CAMP-47318)
