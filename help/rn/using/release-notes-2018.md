...
<table> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> Sinalizador de alta prioridade para o Android - Permite entregar uma notificação por push com alta prioridade para aplicativos do Android, o que faz com que o dispositivo em espera seja ativado e execute um processamento limitado. Observe que a prioridade padrão é Normal, o que pode atrasar a entrega da mensagem para economizar bateria. <br /> Para obter mais informações, consulte a <a href="../../channels/using/customizing-a-push-notification.md#change-the-notification-behavior-for-android">documentação detalhada</a>.<br /> </td> 
  </tr> 
  <tr> 
...
* Um botão **[!UICONTROL Compute stats]** foi adicionado na janela **[!UICONTROL Deployment]** de entregas enviadas. Ele permite recuperar os KPIs mais recentes, por exemplo, se os resultados do envio demorarem muito para serem atualizados ou não tiverem sido considerados. Para obter mais informações, consulte esta [seção](../../sending/using/confirming-the-send.md).
* No fluxo de trabalho técnico predefinido **Atualizar para capacidade de entrega**, os administradores funcionais agora podem definir o número de erros consecutivos a serem ignorados na atividade javascript **Atualizar regras**. Por padrão, o valor do campo é definido como 0, o que significa que todos os erros serão ignorados.
...
* O fluxo de trabalho técnico **[!UICONTROL Update delivery execution]** foi dividido em dois fluxos de trabalho para otimizar o desempenho: - **[!UICONTROL Update delivery execution]**: atualiza o rastreamento de entrega. Ele é iniciado a cada 10 minutos por padrão. **[!UICONTROL Update delivery indicators]**: atualiza os KPIs da entrega, ela é iniciada a cada hora por padrão. Para obter mais informações sobre fluxos de trabalho técnicos, consulte esta [seção](../../administration/using/technical-workflows.md#list-of-technical-workflows).
* Quando uma entrega está enviando mensagens, o status na seção **[!UICONTROL Deployment]** agora pode ter dois valores: **[!UICONTROL Sending]**: as mensagens estão sendo enviadas. **[!UICONTROL Sending (retry)]**: nova tentativa em andamento.
* Usuários com a função **[!UICONTROL Delivery preparation]** agora podem enviar provas. (CAMP-24313)
* A opção **Habilitar TLS em SMPP** foi adicionada ao roteamento de SMS **via SMPP** da conta externa. Para obter mais informações, consulte [esta seção](../../administration/using/configuring-sms-channel.md#defining-an-sms-routing).

**Correções**

* Correção de um problema que impedia o envio de emails ao incluir uma imagem dinâmica do Adobe Target (CAMP-24848).
* Correção de um problema com os fluxos de trabalho técnicos **[!UICONTROL Privacy Access/Delete Request]**, que não eram concluídos se qualquer uma das solicitações falhasse.
...
* Correção de um problema que poderia ocorrer ao classificar dados no menu **[!UICONTROL Client data]**, para recursos personalizados com mais de 100.000 registros. (CAMP-24308)
...
* Correção de um problema que poderia fazer com que determinadas mensagens não fossem consideradas nas métricas **Enviadas**, **Entregues** e **Devolvidas**.
...
* Adicionada uma opção que permite que os KPIs **[!UICONTROL Delivered]** ou **[!UICONTROL Bounces + Errors]** sejam atualizados em tempo real após o envio da entrega. Eles são recalculados diretamente do SR (relatório de status) recebido do provedor.
...
* Correção de um problema que impedia que o vencedor de um email de teste A/B fosse selecionado automaticamente, resultando no não envio do email. Esse comportamento poderia ocorrer se a entrega estivesse no estado **[!UICONTROL retryInProgress]**.
...
* Correção de um problema que impedia que registros de rejeição fossem exibidos em uma transição após uma atividade **Atualizar dados**.
* Correção de um problema que causava a falha do fluxo de trabalho técnico **deliverabilityUpdate**.
...
<table> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> A atividade de fluxo de trabalho <strong>Serviços de assinatura</strong> agora permite assinar ou cancelar a assinatura de uma lista de perfis para vários serviços. No fluxo de trabalho, importe um arquivo contendo os perfis e, para cada perfil, o tipo de operação e o serviço. A atividade de <strong>Serviços de assinatura</strong> poderá usar essas informações e manipular dinamicamente todas as suas assinaturas e cancelamentos de assinatura de perfis de uma só vez.<br /> Para obter mais informações, consulte a <a href="../../automating/using/subscription-services.md">documentação detalhada</a>.<br /> </td> 
  </tr> 
  <tr> 
...
* Correção de um problema que fazia com que a opção **[!UICONTROL Request confirmation before sending messages]** permanecesse marcada mesmo após desmarcá-la ao definir as propriedades de agendamento de uma entrega criada em um fluxo de trabalho.
* A remoção automática de linhas duplicadas (cláusula DISTINCT) agora pode ser desabilitada nas atividades **[!UICONTROL Query]**, por meio de uma nova opção localizada na guia **[!UICONTROL Additional data]**. A desativação dessa opção é recomendada ao definir muitos (mais de 100) elementos adicionais, por motivos de desempenho.

_Integrações_

* Foram feitas algumas melhorias na tela de configuração **[!UICONTROL Data sources]**.
...
* A atividade **[!UICONTROL Scheduler]** agora permite enviar suas entregas dependendo do fuso horário do destinatário.
