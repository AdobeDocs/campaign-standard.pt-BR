...
A execução do mapeamento de dados agora está parada. Você pode usar as APIs do Campaign Standard para acioná-las manualmente.

## Iniciar a ingestão imediata do mapeamento de dados {#starting-immediate-ingestion}
...
>[!NOTE]
>
>Para executar a chamada de API de assimilação POST, o usuário deve ter uma função de execução **de função** SQL, que pode ser fornecida por um administrador do Campaign Standard executando abaixo do Script JS:
>
...
* Falha na solicitação porque o mapeamento XDM não foi publicado ou foi interrompido:
...
>[!NOTE]
Informações detalhadas sobre o status da solicitação de mapeamento XDM e seus trabalhos relacionados estão disponíveis na interface do Campaign Standard, no **!UICONTROL [Status da exportação de dados para o menu da plataforma]** (consulte Ativação [de](../../administration/using/aep-mapping-activation.md)mapeamento).
