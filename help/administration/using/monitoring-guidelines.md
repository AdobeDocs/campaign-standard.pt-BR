...
A interface da Campaign Standard fornece um painel de notificação que permite manter você informado sobre o que está acontecendo no sistema: status de eventos, atualizações do sistema, ação necessária etc. [Leia mais](../../start/using/interface-description.md#top-bar)


**Fluxos de trabalho técnicos**

Eles são operações ou processos programados para serem executados regularmente no servidor. Para garantir que sua instância esteja íntegra e funcionando corretamente, é necessário garantir que elas estejam sempre ativas e em execução. [Leia mais](../../administration/using/technical-workflows.md)
...
O menu **[!UICONTROL Diagnosis]** é uma ferramenta-chave para monitorar e analisar os diferentes objetos técnicos gerados pelo aplicativo: esquemas de dados, páginas da Web, trabalhos em lotes etc. [Leia mais](../../developing/using/monitoring-data-model-changes.md)
